...
let create_node status owner key item = { status; owner; key; item }

let create num def_it =
  {
    heap_lock = Mutex.create ();
    heap = Array.init num (fun _ -> create_node 0 None 42 def_it);
